...
try {
    ...
    for (const id of dungeons) {
        const r = await page.evaluate(async (dungeonId) => {
            ...
            // A directed `bossEdges` counter used to be incremented in that
            // loop and then read by nothing — boss doors are counted below as
            // unique undirected edges instead, which is the number that means
            // something.
